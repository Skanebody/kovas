...
// 3. Try to set is_admin TRUE (column may exist from admin_dashboard migration)
try {
  ...
  if (adminErr) {
    console.log(`is_admin update failed: ${adminErr.message}`)
  } else {
    console.log('is_admin set to TRUE')
  }
} catch (e) {
  console.log(`is_admin skipped: ${e?.message ?? e}`)
}
