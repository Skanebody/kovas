  ...
  return (
    <Card variant="flat" padding="default" id="capture" className="space-y-5">
      <div className="flex items-baseline justify-between gap-3">
        <h2 className="text-[15px] font-semibold text-[#0F1419]">Capture terrain</h2>
        ...
        <div className="space-y-2">
          ...
          <ul className="space-y-2">
            {latestVoice.map((v) => (
              <li
                key={v.id}
                className="rounded-md border border-[#0F1419]/[0.08] bg-cream-deep/40 p-3 text-[13px] text-[#0F1419]/82"
              >
                <p className="line-clamp-2">
                  {v.transcript_raw ?? (
                    <span className="italic text-[#0F1419]/55">Transcription en cours…</span>
                  )}
                </p>
                <p className="mt-1 flex items-center gap-2 text-[11px] text-[#0F1419]/55">
...
function Counter({
  ...
  return (
    <div className="rounded-md border border-[#0F1419]/[0.08] bg-cream-deep/30 p-3">
      <div className="flex items-center gap-2 text-[#0F1419]/72">
        <Icon className="size-3.5" />
        <p className="font-mono text-[10px] uppercase tracking-[0.06em]">{label}</p>
      </div>
      <p className="mt-1 font-serif italic text-[28px] leading-none text-[#0F1419]">{value}</p>
    </div>
  )
}
