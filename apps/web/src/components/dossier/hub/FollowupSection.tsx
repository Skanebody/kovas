  ...
  return (
    <Card variant="flat" padding="default" id="followup" className="space-y-4">
      <div className="flex items-baseline justify-between gap-3">
        <h2 className="text-[15px] font-semibold text-[#0F1419]">Suivi automatique</h2>
      ...
      {upcoming.length > 0 ? (
        <ol className="space-y-2">
          {upcoming.map((it) => {
            ...
            return (
              <li
                key={it.id}
                className="flex items-start gap-3 rounded-md border border-[#0F1419]/[0.08] bg-paper px-3 py-2.5"
              >
                <Icon className="size-4 text-[#0F1419]/72 shrink-0 mt-0.5" />
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline justify-between gap-2">
                    <p className="text-[13px] font-medium text-[#0F1419] truncate">{it.label}</p>
                    <p className="font-mono text-[10px] text-[#0F1419]/55">
                      {new Date(it.at).toLocaleDateString('fr-FR', { dateStyle: 'short' })}
                    </p>
                  </div>
                  {it.description ? (
                    <p className="text-[12px] text-[#0F1419]/72 mt-0.5">{it.description}</p>
                  ) : null}
                </div>
              </li>
            )
        ...
        <div className="rounded-md border border-dashed border-[#0F1419]/[0.08] bg-cream-deep/30 p-4 text-center text-[13px] text-[#0F1419]/72">
          Aucun suivi programmé. Les jalons J+3 / J+7 et les opportunités apparaîtront ici après
          envoi.
        </div>
      )}

      {past.length > 0 ? (
        <details className="text-[12px] text-[#0F1419]/72">
          <summary className="cursor-pointer select-none hover:text-[#0F1419]">
            Historique ({past.length})
          </summary>
          <ul className="mt-2 space-y-1.5">
            {past.map((it) => (
              <li key={it.id} className="flex items-baseline justify-between gap-2">
                <span className="text-[#0F1419]/72">{it.label}</span>
