...
interface AnnuaireTrackGridProps {
  billing: BillingCycle
}

/**
 * Grille des 4 tiers KOVAS Annuaire (B2C lead-gen pour diagnostiqueurs).
 *
 * Spec : `docs/pricing/v3-dual-track-spec.md` §2.
 * Mobile-first : 1 col → sm:2 cols → lg:4 cols.
 */
export function AnnuaireTrackGrid({ billing }: AnnuaireTrackGridProps) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-stretch">
      {ANNUAIRE_PLANS.map((plan) => (
        <AnnuaireCard key={plan.code} plan={plan} billing={billing} />
      ))}
    </div>
  )
}
...
function AnnuaireCard({
  ...
  const isFree = plan.code === 'annuaire_free'
  ...
  const subline = isFree
    ? 'Gratuit à vie · Réclamez votre fiche'
    : billing === 'annual'
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {plan.ficheLevel === 'verified'
            ? 'Coordonnées vérifiées, listing public'
            : plan.ficheLevel === 'premium'
              ? 'Photos, services, tarifs indicatifs'
...
function ctaHrefFor(code: AnnuairePlanCode, billing: BillingCycle): string {
  if (code === 'annuaire_free') return '/reclamer-ma-fiche/find'
  return `/api/stripe/checkout?plan=${code}&cycle=${billing}`
}

function ctaLabelFor(code: AnnuairePlanCode): string {
  if (code === 'annuaire_free') return 'Réclamer ma fiche'
  return 'Choisir'
}
