...
const STATUS_META: Record<LitigationRow['status'], { label: string; variant: 'blue' | 'yellow' | 'green' | 'red' | 'muted' }> = {
        ...
        <div className="space-y-6">
          ...
          <Card variant="opaque" padding="default" className="space-y-4">
            <div className="flex items-center gap-2.5">
              ...
              <h3 className="text-[15px] font-semibold text-ink">Détails du litige</h3>
            </div>
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[13px]">
              <div>
                <dt className="text-[10px] font-mono uppercase tracking-wide text-ink-mute">
                  Type
                </dt>
              ...
              <div>
                ...
                <dd className="font-mono text-ink">{formatDate(litigation.created_at)}</dd>
              </div>
            </dl>
            <div>
              <p className="text-[10px] font-mono uppercase tracking-wide text-ink-mute mb-2">
                Plainte du client
              </p>
              <div className="rounded-md border border-rule bg-sage-alt/60 p-4 text-[13px] text-ink whitespace-pre-wrap leading-relaxed">
          ...
          <Card variant="opaque" padding="default" className="space-y-4">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                <Scale className="size-5 text-ink" />
                <h3 className="text-[15px] font-semibold text-ink">Projet de réponse</h3>
              </div>
            ...
            {litigation.draft_response_md ? (
              <div className="rounded-md border border-rule bg-paper/60 p-4 text-[13px] text-ink whitespace-pre-wrap leading-relaxed font-sans">
                {litigation.draft_response_md}
              </div>
            ) : (
          ...
          {litigation.cited_references && litigation.cited_references.length > 0 ? (
            <Card variant="opaque" padding="default" className="space-y-3">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="text-[15px] font-semibold text-ink">Références citées</h3>
              </div>
          ...
          <Card variant="opaque" padding="default" className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[13px] font-semibold text-ink">Édition manuelle</p>
