 ...
 *
 * Submit :
 *   - Sauvegarder brouillon → redirect `/app/devis/<id>`
...
export function QuoteWizard(props: QuoteWizardProps) {
  ...
  function submitDraft() {
    ...
    startTransition(async () => {
      ...
      router.push(`/app/devis/${res.quoteId}`)
    })
  }
  ...
  function submitSend() {
    ...
    startTransition(async () => {
      ...
      if (!sent.success) {
        ...
        router.push(`/app/devis/${draft.quoteId}`)
        return
      }
        ...
        <Section title="4. Frais de déplacement" eyebrow="LOGISTIQUE">
          {!props.pricingConfig.travelFees ? (
            <p className="text-[13px] text-ink-mute">
              ...
              <Link href="/app/compte/tarifs" className="text-ink underline">
                Compte → Tarifs
              </Link>{' '}
