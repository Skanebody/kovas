...
import { resubmitSirene } from '../actions'
...
interface SireneResubmitFormProps {
  currentSiret: string | null
  currentCompanyName: string | null
  currentLegalForm: string | null
  currentApeCode: string | null
  currentDirector: string | null
}

export function SireneResubmitForm({
  currentSiret,
  currentCompanyName,
  currentLegalForm,
  currentApeCode,
  currentDirector,
}: SireneResubmitFormProps) {
  const [state, formAction, pending] = useActionState(resubmitSirene, undefined)
  const [siret, setSiret] = useState(currentSiret ?? '')
  const [lookingUp, setLookingUp] = useState(false)
  const [sirene, setSirene] = useState<SireneData | null>(
    currentCompanyName
      ? {
          company_name: currentCompanyName ?? undefined,
          legal_form: currentLegalForm ?? undefined,
          ape_code: currentApeCode ?? undefined,
          director_name: currentDirector ?? undefined,
        }
      : null,
  )
        ...
        <div className="rounded-2xl bg-[#F5F7F4] p-5 border border-[#0F1419]/[0.08] space-y-3">
          ...
          <label className="flex gap-3 items-start cursor-pointer pt-2 border-t border-[#0F1419]/[0.06]">
            ...
            <span className="text-[13px] text-[#0F1419]/75">
              ...
              pas à mon identité vérifiée, un document de délégation pourra être demandé.
            </span>
          </label>
        </div>
      )}
