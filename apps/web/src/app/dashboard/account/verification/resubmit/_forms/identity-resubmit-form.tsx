...
import { LivenessCapture } from '@/app/signup/diagnostiqueur/_components/liveness-capture'
import { resubmitIdentity } from '../actions'

type IdentityMethod = 'france_connect' | 'kyc_scan_cni' | 'yousign_qualified'

interface IdentityResubmitFormProps {
  currentMethod: IdentityMethod | null
}

/**
 * Formulaire de re-soumission de la phase Identité.
 * Pré-remplit la méthode actuelle si déjà sélectionnée.
 */
export function IdentityResubmitForm({ currentMethod }: IdentityResubmitFormProps) {
  const [state, formAction, pending] = useActionState(resubmitIdentity, undefined)
  const [method, setMethod] = useState<IdentityMethod>(currentMethod ?? 'france_connect')
