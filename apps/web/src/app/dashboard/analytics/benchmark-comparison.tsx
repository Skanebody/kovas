  ...
  return (
    <Card variant="opaque" padding="none" className="rounded-[24px] overflow-hidden">
        ...
        <table className="w-full text-[12px]">
          <thead className="bg-sage-alt/40">
            <tr className="text-left">
              <th className="font-mono text-[10px] uppercase tracking-[0.1em] font-medium text-ink-mute px-6 py-2.5">
                Métrique
              </th>
              <th className="font-mono text-[10px] uppercase tracking-[0.1em] font-medium text-ink-mute px-3 py-2.5 text-right">
                Vous
              </th>
              <th className="font-mono text-[10px] uppercase tracking-[0.1em] font-medium text-ink-mute px-3 py-2.5 text-right hidden sm:table-cell">
                Région
              </th>
              <th className="font-mono text-[10px] uppercase tracking-[0.1em] font-medium text-ink-mute px-3 py-2.5 text-right">
                FR
              ...
              <th className="font-mono text-[10px] uppercase tracking-[0.1em] font-medium text-ink-mute px-6 py-2.5 hidden md:table-cell">
                Position
              </th>
            </tr>
          </thead>
                ...
                <tr key={row.id} className="hover:bg-ink/[0.02] transition-colors">
                  ...
                  <td
                    className={cn(
                      ...
                      aboveNational ? 'text-success' : 'text-ink',
                    )}
                  >
                    {formatValue(row.yourValue, row.unitSuffix)}
                  </td>
                  <td className="px-3 py-3 text-right font-mono tabular-nums text-ink-mute hidden sm:table-cell">
                    {formatValue(row.regional ?? null, row.unitSuffix)}
                  </td>
                  <td className="px-3 py-3 text-right font-mono tabular-nums text-ink-mute">
                  ...
                  <td className="px-6 py-3 hidden md:table-cell">
                    {pos == null ? (
                      ...
                      <div
                        className="relative h-1.5 w-full max-w-[120px] rounded-full bg-rule/40"
                        aria-label={`Position ${Math.round(pos)}%`}
                      >
                        <div
                          className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 size-2.5 rounded-full bg-ink"
                          style={{ left: `${pos}%` }}
                          aria-hidden
                        />
                      </div>
      ...
      <div className="border-t border-rule/60 px-6 py-3">
        <p className="font-mono text-[10px] text-ink-mute italic leading-relaxed">
          {hint ?? defaultHint}
        </p>
      </div>
    </Card>
