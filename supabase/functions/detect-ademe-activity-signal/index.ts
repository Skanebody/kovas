...
async function fetchAdemeByCertificate(certNumber: string, fromDate: string): Promise<AdemeLine[]> {
...
async function fetchAdemeByName(
  ...
  const url = new URL(ADEME_ENDPOINT)
  url.searchParams.set('q_fields', 'nom_du_diagnostiqueur,prenom_du_diagnostiqueur')
  ...
  if (!res.ok) {
    throw new Error(`ADEME HTTP ${res.status} (name=${lastName} ${firstName})`)
  }
  const data = (await res.json()) as AdemeLineResponse
  return data.results ?? []
}
...
async function detectForDiagnostician(
  ...
  try {
    ...
    if (merged.size === 0 && diagnostician.first_name && diagnostician.last_name) {
      ...
      for (const line of lines) {
        const key = line.numero_dpe ?? `${diagnostician.last_name}:${line.date_etablissement_dpe}`
        if (!merged.has(key)) merged.set(key, line)
      }
      await sleep(RATE_LIMIT_DELAY_MS)
    }
...
async function handleRequest(req: Request): Promise<Response> {
  ...
  if (mode === 'single') {
    ...
    const { data, error } = await supabase
      ...
      .single()
    if (error || !data) {
      return jsonResponse({ ok: false, error: 'diagnostician_not_found' }, 404)
    }
    targets = [data as DiagnosticianRow]
  } else {
    const requestedLimit = Math.min(Math.max(body.limit ?? DEFAULT_BATCH_LIMIT, 1), MAX_BATCH_LIMIT)
    const cutoff = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString()
    const { data, error } = await supabase
      .from('diagnosticians')
      .select('id, first_name, last_name')
      .or(`ademe_last_synced_at.is.null,ademe_last_synced_at.lt.${cutoff}`)
  ...
  for (const diagnostician of targets) {
    const detection = await detectForDiagnostician(supabase, diagnostician, fromDate)
