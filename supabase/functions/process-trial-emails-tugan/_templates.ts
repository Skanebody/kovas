/**
 * KOVAS — Tugan trial sequence (8 emails J+0..J+30) — copie Deno-friendly.
 *
 * Source canonique : `apps/web/src/lib/email/tugan-trial-sequence.ts` (TUGAN-4).
 * Cette copie est nécessaire car les Edge Functions Deno ne peuvent pas
 * importer le module Next.js (paths, tsconfig, build). Toute modification
 * d'un template DOIT être synchronisée entre les deux fichiers.
 *
 * Validation cross-source : un test Vitest (TUGAN-EMAIL-WIRING bonus) peut
 * comparer les `code`, `dayOffset` et `placeholders` pour détecter les drifts.
 *
 * AUCUNE mention de provider IA tiers dans le code source de cette fonction
 * (directive transversale 2026-05).
 */
...
/** 8 templates ordonnés par dayOffset croissant — index = step - 1. */
 ...
 * Validation stricte : un placeholder déclaré manquant → erreur explicite.
 */
