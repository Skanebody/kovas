...
serve(async (req) => {
  ...
  try {
    ...
    const keywords = Array.from(
      new Set([...SEED_KEYWORDS_50, ...(body.additionalKeywords ?? [])].map((k) => k.trim())),
    ).filter((k) => k.length > 0)

    const apiKey = Deno.env.get('SERPAPI_API_KEY') ?? ''
    const endpoint = Deno.env.get('GOOGLE_TRENDS_API_URL') ?? 'https://serpapi.com/search.json'
    const throttleMs = Number.parseInt(Deno.env.get('SERPAPI_THROTTLE_MS') ?? '1200', 10)
    ...
    for (const keyword of keywords) {
      try {
        ...
        if (mock) {
          ...
          trendScore = await fetchSerpapiTrendScore(apiKey, endpoint, keyword, geo)
